...
[`scripts/qupath-utils/atlas/addAtlasCoordinates`](https://github.com/TeamNCMC/cuisto/blob/main/scripts/qupath-utils/atlas/addAtlasCoordinates.groovy) to get the atlas coordinates on each Detections, which is necessary if you want to create a distribution graph with `cuisto`. 
